...
uint16_t q3(uint8_t x, uint8_t y) {
    ...
    uint8_t togglevar = 10000001;
    uint16_t solution = x ^ togglevar;
    return (solution << 8) | y;
}
...
int32_t q4(uint8_t * array, uint32_t arrayLength) {
    ...
    int32_t sum = 0;
    if(sizeof (array)/sizeof(uint8_t) < arrayLength) arrayLength = sizeof (array)/sizeof(uint8_t);
...
error_t q10(q10_t *q10) {
    if(q9(&q10->a,&q10->b) ==0) return SUCCESS;
    else return FAIL;
}
...
error_t q11(q11_a_t *a, q11_b_t *b){
    ...
    memcpy(&b->array[1],&a->array[0], sizeof(uint16_t)*50);
    return SUCCESS;
}
...
void *q13(uint32_t *ptr1, uint16_t *ptr2){
    ...
    uint32_t address1 = (uint32_t) ptr1, address2 = (uint16_t) ptr2;
    void *solution = (int*) (MIN(address1, address2)+5);
    return solution;
}
//-------------------------------------------------------------------------
